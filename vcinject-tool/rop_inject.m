...
void injectDylibViaRop(task_t task, pid_t pid, const char* dylibPath, vm_address_t allImageInfoAddr)
{
	...
	if(remoteDylibPath)
	{
		// dlopenRet initialisieren, sonst ist die Erfolgsprüfung unbestimmt (Astra-Fix).
		uint64_t dlopenRet = 0;
		kern_return_t dlopenKr = arbCall(task, pthread, (uint64_t*)&dlopenRet, true, dlopenAddr, 2, remoteDylibPath, RTLD_NOW);
		vm_deallocate(task, remoteDylibPath, remoteDylibPathSize);

		// arbCall-Rückgabestatus explizit prüfen (nicht nur den Handle-Wert).
		printf("[injectDylibViaRop] dlopen arbCall kr=%d handle=0x%llX\n", dlopenKr, (unsigned long long)dlopenRet);

		if (dlopenKr == KERN_SUCCESS && dlopenRet) {
			printf("[injectDylibViaRop] dlopen succeeded, library handle: 0x%llX\n", (unsigned long long)dlopenRet);

			// NIKE CAM PATCH: _dlopen ist die normale Mach-O-Schreibweise der C-Funktion
			// dlopen und fuehrt Initializer/ctor aus. Falls der ctor ausbleibt (z.B. weil
			...
			if (startFunc) {
				...
				// void-Funktion: startRet (x0) ist kein Erfolgscode, nur arbCall-Status zählt.
				printf("[injectDylibViaRop] VCamInject_start arbCall kr=%d\n", startKr);
			} else {
				printf("[injectDylibViaRop] WARN: _VCamInject_start not found (Symbol nicht in LC_SYMTAB?)\n");
			}
		}
		else {
			uint64_t remoteErrorString = 0;
			kern_return_t errKr = arbCall(task, pthread, (uint64_t*)&remoteErrorString, true, dlerrorAddr, 0);
			if (errKr == KERN_SUCCESS && remoteErrorString) {
				char *errorString = task_copy_string(task, remoteErrorString);
				printf("[injectDylibViaRop] dlopen failed, error:\n%s\n", errorString);
				free(errorString);
			} else {
				printf("[injectDylibViaRop] dlopen failed (kr=%d, kein dlerror lesbar)\n", dlopenKr);
			}
		}
	}
